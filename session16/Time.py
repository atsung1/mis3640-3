...
def increment_2(time, seconds):
    ...
    dummytime = Time()
    dummytime.second = time.second
    dummytime.minute = time.minute
    dummytime.hour = time.hour


    dummytime.second += seconds

    if dummytime.second >= 60:
        dummytime.second -= 60
        dummytime.minute += 1

    if dummytime.minute >= 60:
        dummytime.minute -= 60
        dummytime.hour += 1
...
def mul_time(t1, factor):
    ...
    if valid_time(t1):
        seconds = factor * time_to_int(t1)
        t1 = int_to_time(int(seconds))
        return t1
    else:
